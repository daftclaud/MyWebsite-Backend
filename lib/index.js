...
exports.twilioAssistant = functions.https.onRequest(async (req, response) => {
    const incoming = req.body.Body;
    if (!isNaN(+incoming)) {
        const choice = +incoming;
        const defsRef = await fs.doc('Functions/Storage').get();
        const defsData = defsRef.data();
        const word = defsData ? defsData.word : '';
        const type = defsData ? defsData.type : '';
        const defs = defsData ? defsData.definitions : [];
        ...
        const type = incoming.split(' ')[1].toLowerCase();
...
const getTextResponse = ({ body, type }) => {
